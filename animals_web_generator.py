...
def serialize_animal(animal_obj):
    ...
    animal_scientific_name = animal_obj['taxonomy'].get('scientific_name')
    ...
    animal_location = animal_obj["locations"][0]
    output = ""
    ...
    output += "<ul>\n\t\t\t"
    output += f"<li><strong>Scientific Name:</strong>{animal_scientific_name}</li>\n\t\t\t"
    output += f"<li><strong>Diet:</strong> {animal_diet}</li>\n\t\t\t"
    output += f"<li><strong>Location:</strong> {animal_location}</li>\n\t\t\t"
...
def animals_data_not_found(animal_name):
    """ This function create a new site if the animal searched doesn't exist"""
    output = ""
    output += f"<h2><strong>Error Message:</strong> Oops! Animal * {animal_name} * Not Found</h2>\n"
    output += "<li class='cards__item'>\n\t"
...
def get_animal_name_from_user():
    """ This function ask the user to insert validate animal name """
    while True:
        try:
            animal_name = input("Enter a name of an animal: ")
            if animal_name == "" or animal_name.isdigit() or len(animal_name) < 2:
